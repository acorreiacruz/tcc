...
import FailDelivery from "../../src/application/use_case/failDelivery";
import { DeliveryPersonDoesNotMatchError } from "../../src/application/use_case/errors";

describe("Testing FailDelivery", () => {
    ...
    const failDelivery = new FailDelivery(unitOfWork);
    ...
    test("Must fail a delivery", async () => {
        ...
        expect(updatedDelivery.getStatus()).toBe("failed");
    });
    
    test("Must throw error when delivery person does not match", async () => {
        await expect(
            failDelivery.execute({
                deliveryId: delivery.getId(),
